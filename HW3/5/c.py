...
def recover_image(img,h,landa):
    ...
    H = np.array(H)
    y = img_dfft(img)
    x = (H.conj() * y) / (H.conj() * H + landa*(A.conj() * A))
    x = img_idft(x)
    return x
...
if __name__ == "__main__":
    ...
    for i in range(len(blured_trump1)):
        plt.subplot(2, 2, i + 1)
        x = recover_image(blured_trump1[i], kernels[i], landa)
    ...
    for i in range(len(blured_trump2)):
        plt.subplot(2, 2, i + 1)
        x = recover_image(blured_trump2[i], kernels[i], landa)
        plt.imshow(x, cmap="gray")
