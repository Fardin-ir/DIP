...
def get_sift_homography(img1, img2):
    ...
    verify_ratio = 0.9  # Source: stackoverflow
    verified_matches = []
    ...
    if len(verified_matches) > min_matches:
        ...
        M, mask = cv2.findHomography(img1_pts, img2_pts, cv2.RANSAC,1)
        return M, mask
...
if __name__ == '__main__':
    import glob
    image_list = []
    for filename in glob.glob('P3/III/*.png'):
        print(filename)
        im = cv2.imread(filename)
        im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        image_list.append(im)

    res = main(image_list[:6])
