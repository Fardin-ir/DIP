...
if __name__ == "__main__":
    img = cv2.imread("P4/post-mortem_3.png")
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    mask = np.zeros(img.shape[:2])
    mask[0:230,0:200] = cv2.cvtColor(img,cv2.COLOR_RGB2GRAY)[0:230,0:200]
    mask[mask>250] = 1
    mask[mask!=1] = 0
    kernel = np.ones((20, 20), np.uint8)
    mask = cv2.dilate(mask, kernel, iterations=1)
    img = cv2.inpaint(img, mask.astype("uint8"), 20, cv2.INPAINT_TELEA) / 255
    plt.imshow(mask)
    plt.show()
    img = wgif(img, img, 16, 16, 0.4 ** 2)
    plt.imshow(img)
    plt.show()
